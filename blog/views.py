    ...
    if len(words)>0:
        ...
        for word in words:
            if word in POSITIVE_WORDS:
                count += 1
            elif word not in NEGATIVE_WORDS:
                count += .5
        return count/len(words)
    

def calcSentimentTrend(text):
    text = text.split('. ')
    trend = [calcSentiment(yo) for yo in text]
    return trend

def calcAverageSentiment(trend):
    if len(trend)>0:
...
def results(request):
    if request.method == 'POST':        
        ...
        # NOTE: calcComplexityVariance returns array of [avg_len, text_std, max_length, min_length]
